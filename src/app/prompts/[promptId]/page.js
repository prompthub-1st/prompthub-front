...
import { deletePrompt, fetchAllData } from "@/api/promptService";
...
export default function PromptDetailPage() {
  ...
  const prompt = allData.find(
    (p) => String(p.promptId) === String(promptId)
  );
  ...
  const userName = prompt.userName || '익명';
  const categoryName = prompt.categoryName || '미분류';
    ...
    <div className={styles.container}>
      <article>
        <div className={styles.header}>
          ...
          {loginUser &&
            String(loginUser.userId) === String(prompt.userId) && (
